...
namespace NT_RFIDService
{
    public class Reader
    {
        ...
        private string LogURL = "http://192.168.100.101:8069/peacebird/upload_log?db=peacebird_demo";
        ...
        public Reader(string URL, string LogURL, string IP, int Port, byte fComAdr, int OUT_DURATION, int IN_DURATION, int SleepTime)
        {
            try
            {
                this.URL = URL;
            ...
            log.Info("ConfigInfo: {URL:" + URL + ", LogURL:" + LogURL + ", IP:" + IP + ", Port:" + Port + ", fComAdr: " + fComAdr + ", OUT_DURATION: " + OUT_DURATION + ", IN_DURATION: " + IN_DURATION + ", SleepTime: " + SleepTime);
        }
        ...
        public void Start()
        {
            log.Info("in Start() ConfigInfo: {URL:" + URL + ", LogURL:" + LogURL + ", IP:" + IP + ", Port:" + Port + ", fComAdr: " + fComAdr + ", OUT_DURATION: " + OUT_DURATION + ", IN_DURATION: " + IN_DURATION + ", SleepTime: " + SleepTime);
        ...
        public bool OpenConnect()
        {
            ...
            while (ret != 0)
            {
                stateCode = ret + ":" + GetReturnCodeDesc(ret);
            ...
            while (true)
            {
                if (epcTable.Count > 0)
                {
                    ...
                    if (fittingList != null && fittingList.Count() > 0)
                    {
                        ...
                        // 向服务器发送数据
                        Thread thread = new Thread(Uploader.uploadReaderData);
        ...
        {
            log.Info("in LogThread() ConfigInfo: {URL:" + URL + ", LogURL:" + LogURL + ", IP:" + IP + ", Port:" + Port + ", fComAdr: " + fComAdr + ", OUT_DURATION: " + OUT_DURATION + ", IN_DURATION: " + IN_DURATION + ", SleepTime: " + SleepTime);

            while (true)
            {
                // 每3分钟上传一次设备连接状态信息
                Thread.Sleep(180000);
                Thread thread = new Thread(Uploader.uploadLogData);
                thread.Start(new Object[3] { LogURL, stateCode, deviceNo });
                ...
                if ((ret == 1) || (ret == 2) || (ret == 0x26))//代表已查找结束，
                {
                    ...
                    for (int CardIndex = 0; CardIndex < CardNum; CardIndex++)
                    {
                        ...
                        if (epc != null)
                        {
                            ...
                            // 如果保存可更新结束时间
                            epc.setEndDatetime(DateTime.UtcNow.ToString(DATEFORMAT));
                        }
                        else {
                            // 如果没有保存创建新的对象并且保存
                            epcTable.Add(sEPC, new EPC(deviceNo, lineNo, sEPC, DateTime.UtcNow.ToString(DATEFORMAT), DateTime.UtcNow.ToString(DATEFORMAT), RSSI, 0, RSSI, RSSI));
                        }
                    }
                }
                else {
        ...
        private List<EPC> getFittingData(Hashtable epcMap)
        {
            ...
            foreach (String key in epcMap.Keys)
            {
                ...
                try
                {
                    ...
                    if ((DateTime.UtcNow - endDate).TotalSeconds > OUT_DURATION)
                    {
